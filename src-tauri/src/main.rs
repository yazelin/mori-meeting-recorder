...
fn main() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            // recorder 是普通 app:不長自己的 tray(✕ 關閉就退出;視窗在工作列可達)。
            // 「啟動 / 部署」這類 hub 行為交給 mori-desktop(未來 AgentOS)的 tray —— 它偵測到
            // 本 recorder 安裝就在自己的 tray 出「會議錄音」啟動本 app(見 mori-desktop 端)。
